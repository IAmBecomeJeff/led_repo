...
void FireworkEffect::prepare_for_explosion(){
    if(DEBUG){Serial.println("Prepare for Explosion");}
...
void FireworkEffect::fadeup(){
  if(DEBUG){Serial.println("Stage: FADEUP");}
    if(exploded){
        ...
        if(DEBUG){Serial.print("firework_bri: "); Serial.println(firework_bri);}
        if(firework_bri >= 255){
            exploded = true;
        }
    }
...
void FireworkEffect::explosion(){
  if(DEBUG){Serial.println("EXPLOSION!");}
