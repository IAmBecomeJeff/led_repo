...
void change_mode(uint8_t mn = 255) {
	...
	if (mn == 255) {					// large number to avoid interfering with read_keyboard mode sends
		if (random_mode) { next_leds.mode_number = (next_leds.mode_number = random8(ARRAY_SIZE(ModeList))); Serial.print("Random Mode number: "); Serial.println(next_leds.mode_number); }
