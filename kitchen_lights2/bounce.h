...
void bounce(LEDStruct& leds) {
	...
	if (leds.bounce_rainbow) { fill_rainbow(leds.led_data, NUM_LEDS, leds.rainbow_index++, leds.rainbow_diff); }
	else { fill_palette(leds.led_data, NUM_LEDS, leds.rainbow_index++, leds.rainbow_diff, leds.current_palette, 255, LINEARBLEND); }
	...
	EVERY_N_MILLIS(25) { leds.bounce_start = beatsin8(leds.bounce_start_beat, 0, NUM_LEDS - leds.bounce_length); }
}
