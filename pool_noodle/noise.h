...
void noise_init(LEDStruct& leds, bool ufr = random8(2), uint16_t s = random16(10,40), uint16_t y = random16(10,40), uint16_t d = random16(1000,50000)) {
...
void noise_mover(LEDStruct& leds) {
	if (!leds.mode_initialized) { nosie_init(leds); }
	if (keyboard_update) { noise_update(leds); }

	uint8_t locn = inoise8(leds.noise_scale, leds.dist + leds.noise_yscale) % 255;          
	uint8_t pixlen = map(locn, 0, 255, 0, leds.strip_range);     
	leds.led_data[pixlen] = ColorFromPalette(leds.currentPalette, pixlen, 255, LINEARBLEND);   

	leds.dist += beatsin8(10, 1, 4);
