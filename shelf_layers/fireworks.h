...
#define MAX_NUMBER_OF_SPARKS 30
#define MIN_NUMBER_OF_SPARKS 10
...
void prepare_for_explosion() {
	...
	firework_position = random8(20, shelf_num_leds[0] - 10);
	current_stage = FADEUP;
	firework_hue = random8();
	firework_bri = 0;
	exploded = false;
	fill_solid(leds, shelf_num_leds[0], CRGB::Black);
}
...
void fadeup() {
	if (exploded) {
		...
		leds[shelf[0][firework_position]] = CRGB::White;
	}
	else {
		firework_bri += 5;
		leds[shelf[0][firework_position]] = CHSV(firework_hue, 255, firework_bri);
		if (firework_bri >= 255) {
			exploded = true;
		}
	}
...
void firework() {
	if (mode_change) {
		...
		this_delay = 10;
		current_stage = WAITING;
		firework_position = random8(10, shelf_num_leds[0] - 10);
		firework_hue = random8();
